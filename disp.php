...
$tmp=mysqli_query($db, "SELECT Fname from employee where P_no='$pn'");
$no=mysqli_fetch_object($tmp);
if($no=""){$pno="";}else{$pno=$pn;}

$query="SELECT Quantity-$quo FROM product where P_id='$pid'";
$q=mysqli_query($db, $query);

$query1="UPDATE product SET Quantity=Quantity-$quo  WHERE (Quantity-$quo)>-1 AND P_id='$pid'";
...
if(empty($pno)|| empty($date)||empty($pid)|| empty($sno)||empty($loc)|| empty($quo)||empty($dept)){
...
<!--End of web_dialog3-->
			...
			<input type="submit" id="send" value="submit" style="margin-top:125px;">
             </form>
